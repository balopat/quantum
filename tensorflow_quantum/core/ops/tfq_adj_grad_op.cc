...
class TfqAdjointGradientOp : public tensorflow::OpKernel {
 public:
  ...
  void Compute(tensorflow::OpKernelContext* context) override {
    ...
    std::vector<std::vector<std::vector<qsim::GateFused<QsimGate>>>> partial_fused_circuits(
        programs.size(), std::vector<std::vector<qsim::GateFused<QsimGate>>>({}));
    // track gradient info.
    std::vector<std::vector<tfq::GradientOfGate>> gradient_gates(
      programs.size(), std::vector<tfq::GradientOfGate>({}));


    auto construct_f = [&](int start, int end) {
      for (int i = start; i < end; i++) {
        OP_REQUIRES_OK(context, QsimCircuitFromProgramADJ(
                                    programs[i], maps[i], num_qubits[i],
                                    &qsim_circuits[i], &partial_fused_circuits[i],
                                    &gradient_gates[i]));
      }
    };
    ...
    OP_REQUIRES(context, downstream_grads[0].size() == pauli_sums[0].size(),
      tensorflow::errors::InvalidArgument(absl::StrCat(
                    "Number of gradients and pauli sum dimension do not match. Got ",
                    downstream_grads[0].size(), " gradient entries and ", pauli_sums[0].size(),
                    " paulis per circuit.")));
    ...
    ComputeLarge(
      num_qubits, qsim_circuits, maps, partial_fused_circuits, pauli_sums,
      gradient_gates, downstream_grads, context, &output_tensor);

    // just to be on the safe side.
    qsim_circuits.clear();
    partial_fused_circuits.clear();
 ...
 private:
  void ComputeLarge(
      ...
      const std::vector<std::vector<std::vector<qsim::GateFused<QsimGate>>>>& partial_fused_circuits,
    ...
    for(int i =0;i<partial_fused_circuits.size();i++){
      ...
      for (int j = 0; j < partial_fused_circuits[i].size(); j++) {
        ...
        qsim::ApplyGate(sim, qsim_circuits[i].gates[gradient_gates[i][j].index], sv);
      }

      // sv now contains psi
      // scratch contains (sum_i paulis_sums[i] * downstream_grads[i])|psi>
      AccumulateOperators(
        pauli_sums[i], downstream_grads[i], sim, ss, sv, scratch);
      ...
      for (int j = partial_fused_circuits[i].size() - 1; j>=0; j--) {
        ...
        ApplyGateDagger(sim, qsim_circuits[i].gates[gradient_gates[i][j - 1].index], sv); 
        for(int k =0;k<gradient_gates[i][j - 1].grad_gates.size();k++){
          // Copy sv onto scratch2 in anticipation of non-unitary "gradient gate".
        ...
        ApplyGateDagger(sim, qsim_circuits[i].gates[gradient_gates[i][j - 1].index], scratch); 
      }
    }
    sv.release();
    scratch.release();
